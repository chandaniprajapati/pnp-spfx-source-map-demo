...
(async function build() {
  ...
  /**
   * Get the JS files from `./dist/Angualr11SourceMap/` directory and combine it into `bundle.js`
   * This will give support for angular v6 and all above versions.
   */
 
  try {
    let directoryPath = './dist/Angualr11SourceMap/';
    let files = await getFilesFromDirByType(directoryPath, 'js');
    files = files.map(file => directoryPath + file);
    await concat(files, './dist/Angualr11SourceMap/bundle.js');
  } catch (error) {
    console.log(error);
  }
})();
